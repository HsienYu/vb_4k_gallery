...
DIALOG=dialog
BACKTITLE="Armbian Bionic RK3288 media testing script (v2.0)"
...
SELECTION=$($DIALOG --backtitle "$BACKTITLE" \
        ...
        "Devel" "Install development libraries" on\
        "MPV" "MPV player 0.28 with EGL/GBM+RKMPP acceleration" on \
        "Gstreamer" "Gstreamer Rockchip plugins and Qt player" on \
        "CLSamples" "OpenCL examples: Arm CL and GPU crypto miner" off \
        ...
        "Kodi" "RKMPP+GBM accelerated Kodi 18.0-alpha (preview)" off \
        2>&1 1>&3)
...
then
	...
	XSERVER=$($DIALOG --stdout --backtitle "$BACKTITLE" --title "Alternate X server" \
	        --menu "By default, the script will install a stable version of Rockchip X server, providing decent performance.\nAlternatively, you can install an experimental version (under testing), with some tweaks that may improve performance.\nWhich version do you prefer?" \
	        18 60 12 Sta "Tested and stable, laggy desktop" Exp "Experimental, possibly better desktop")
fi
...
then
	# Stable X server
	dpkg --unpack packages/xserver/sta/*.deb
else
	# Experimental X server
	dpkg --unpack packages/xserver/exp/*.deb
fi
apt-get -y -f install
...
then
	echo '*************************************************** 
*    Installing Web streaming support libraries   *
***************************************************'
	dpkg --unpack packages/streaming/*.deb
	apt-get -f -y install
fi

# Install OpenCL examples
if [ $CLSAMPLES -eq 1 ]
then
	echo '*************************************************** 
...
then
	echo '*************************************************** 
*     Installing accelerated Kodi 18.0-alpha      *
***************************************************'
	sudo usermod -aG input $DEFAULTUSER # Workaround until this fix is included in stable Armbian
	apt-get -y purge *kodi*
	apt-get -y --allow-downgrades install ./packages/kodi/deps/*.deb
	apt-get -y --allow-downgrades install ./packages/kodi/*.deb
	[ $DEVEL -eq 1 ] && apt-get -y --allow-downgrades install ./packages/kodi/deps/dev/*.deb
	[ $DEVEL -eq 1 ] && apt-get -y --allow-downgrades install ./packages/kodi/dev/*.deb
fi
